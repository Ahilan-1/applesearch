...
class AppleSearch:
    ...
    def _extract_info_box(self, soup):
        ...
        if info_box:
            try:
                title = info_box.find('h2', {'class': 'qrShPb'})
                description = info_box.find('div', {'class': 'LGOjhe'})
                image = info_box.find('g-img')
                
                return {
                    'title': title.get_text() if title else '',
                    'description': description.get_text() if description else '',
                    'image_url': image.img['src'] if image and image.img else None,
                    'type': 'info_box'
                }
    ...
    def _rank_results(self, query, results):
        ...
        for result in results:
            if result['type'] == 'info_box':
                result['score'] = 100  # Prioritize info boxes
                continue
        ...
        # Sort results by score, keeping info box at top if present
